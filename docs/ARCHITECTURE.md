...
- `src/analysis/smoothing.ts` derives timestamp-aware local quadratic position/velocity/acceleration at confirmed observation timestamps; `modelFit.ts` fits and evaluates global constant-velocity or constant-acceleration descriptions.
- `src/analysis/panelState.ts` owns the small pure collapse, quantity-family, analysis-source/window, and model state machine for the Analysis panel; it never owns or copies derived sample data.
...
Save builds a fresh snapshot from current domain state, validates it through the same parser boundary, and downloads formatted JSON. Data export calls `deriveTrackKinematics` for every track, preserving the application’s raw timestamp and derivative policy regardless of the session-only Phase 9 view. CSV rows are globally chronological and carry stable IDs, frame references, native pixels, generic analysis values, and explicit unit fields. JSON data export omits workspace-only state. Graph export runs the current layered visualization group through the tested chart layout and emits a self-contained SVG with title, legend, axes, units, measured/smoothed/model layers as applicable, but no playhead, cursor, or hit targets.
