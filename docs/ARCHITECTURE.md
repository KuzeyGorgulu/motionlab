...
Save builds a fresh snapshot from current domain state, validates it through the same parser boundary, and downloads formatted JSON. Data export calls `deriveTrackKinematics` for every track, preserving the application’s timestamp and derivative policy. CSV rows are globally chronological and carry stable IDs, frame references, native pixels, generic analysis values, and explicit unit fields. JSON data export omits workspace-only state. Graph export runs the existing visualization group through the tested chart layout and emits a self-contained SVG with title, legend, axes, units, and sample markers but no playhead, cursor, or hit targets.
