...
- `src/App.tsx` owns whether a local video is selected, switches between the empty and active workspaces, and retains the in-memory acknowledgement of the assisted-tracking experimental notice across video replacement.
...
For reports, `VideoWorkspace` prepares one raw or selected smoothed analysis per track using the existing analysis functions, then passes any existing model fit and `FitDiagnostics` into `buildExperimentReport`. The builder only aggregates/selects: endpoint displacement divided by duration defines average velocity, existing cumulative distance divided by duration defines average speed, and maximum values select existing finite derived magnitudes. `graphs.ts` filters existing `VisualizationGroup`s rather than implementing a chart. Both the React document and `reportHtml.ts` consume the same typed model; HTML strips no data from the DOM, contains embedded CSS/SVG, has no script/network dependency, and excludes the video.
