...
Smoothing, model fitting, multi-track graph comparison, interpolation, persistence, perspective correction, assisted tracking, and export do not exist in the current codebase. Annotation and tracking undo histories are in-memory and use bounded full immutable snapshots; a command/delta model can replace them if large projects demonstrate a measured need. Calibration currently has no undo history and must be reset or edited explicitly. Timestamp buckets remain an approximate fallback identity until a decoded-frame-aware strategy is available.
