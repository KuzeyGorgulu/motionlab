...
- `src/components/tracking/AssistedTrackingNotice.tsx` presents the accessible first-use experimental notice without entering assisted-tracking domain state.
- `src/project/types.ts`, `schema.ts`, `serialize.ts`, and `videoRelink.ts` define the versioned project boundary, safe parser, snapshot serializer, filename policy, and lightweight media comparison as pure tested modules.
- `src/export/scientificData.ts`, `graphSvg.ts`, `reportHtml.ts`, and `download.ts` derive interoperable data, render standalone graphs/reports, and perform the final browser download without duplicating analysis formulas.
- `src/components/project/` contains compact open/save/export controls plus relink and mismatch states.
...
- `src/analysis/leastSquares.ts` provides the small deterministic partial-pivot solver and normalized polynomial regression shared by Phase 9 analysis.
- `src/analysis/smoothing.ts` derives timestamp-aware local quadratic position/velocity/acceleration at confirmed observation timestamps; `modelFit.ts` fits and evaluates global constant-velocity or constant-acceleration descriptions; `fitDiagnostics.ts` derives observation-aligned residual metrics, rankings, and conservative visual outlier flags.
- `src/analysis/panelState.ts` owns the small pure collapse, motion/residual view, quantity/component, analysis-source/window, and model state machine for the Analysis panel; it never owns or copies derived sample data.
- `src/components/analysis/KinematicsPanel.tsx` presents compact current-sample quantities plus model diagnostics and actionable largest deviations in the right inspector.
...
- `src/report/types.ts`, `projectState.ts`, `builder.ts`, and `graphs.ts` define validated persisted report preferences and the pure project/scientific-state → report-model projection. `src/components/report/ReportWorkspace.tsx` renders the editable configuration and print document without scraping application DOM.
...
- `src/product/version.ts` exposes the authoritative `package.json` version to About and report provenance; `preferences.ts`, `content.ts`, and `sample.ts` isolate the local first-run preference, documented product content, and normal sample-project fetch/parse boundary.
- `src/components/product/` contains reusable native-dialog focus management, compact first-run onboarding, and Help topics for shortcuts, examples, Privacy, and About. These components do not import or mutate scientific reducers.
...
React component state is used for transient media/UI state. Annotation domain data uses a reducer, while unfinished drafts and drag previews remain transient. A global state library would not add value. Live experiment data is session-scoped until the user explicitly downloads a project. Workflow guidance is recomputed from existing facts and native `<details>` elements own only ephemeral disclosure state. First-run completion is the sole product preference stored in `localStorage`; it is versioned, failure-tolerant, and excluded from project/scientific state. Residual views, diagnostics, rankings, and outlier flags are derived/session-only. The assisted-tracking experimental acknowledgement remains a separate in-memory `App` flag and is not stored in tracking history, browser storage, or telemetry.

Calibration has a separate reducer. `VideoWorkspace` is keyed by the local Object URL, so video replacement remounts all video-scoped domain controllers; the calibration and tracking reducers additionally define explicit `video-replaced` reset transitions for testability. Calibration is included only in explicit project saves and is not placed in annotation or tracking undo history.
...
`MotionLabProjectV1` is an explicit JSON boundary with `format: "motionlab"` and `version: 1`. Its video reference contains a name and optional native dimensions/duration; no video bytes, Object URL, or browser file handle are serialized. Domain arrays contain annotations, calibration, tracks, and accepted samples. Workspace metadata is limited to active track, trail mode, advance-after-mark, analysis mode/expansion, and media time. Phase 12 adds an optional-on-read, required-after-normalization `report` section containing bounded user text and stable report preferences only. Old files without the section receive deterministic defaults; invalid graph values or stale track references are rejected. Derived report observations, summaries, fits, diagnostics, graphs, and generation timestamps are never serialized.

`App` owns parsed-but-not-yet-relinked project state. Opening reads and validates the complete file before prompting to discard meaningful current data. A valid open unmounts the old video workspace and shows the expected video name. The selected video creates a new keyed `VideoWorkspace`; validated project values initialize the annotation, calibration, tracking, and analysis reducers in one mount. Filename, resolution, and duration mismatches keep relinking pending until the user chooses another file or explicitly accepts the warning. Invalid JSON/schema/version never reaches reducer state.

The bundled sample follows this same boundary: `App` fetches a separate project text and small WebM, parses the project with `parseMotionLabProject`, creates a normal local `File`, and mounts the regular keyed/relinked `VideoWorkspace`. It has no sample-only analysis, fitting, calibration, report, or serialization branch.

Save builds a fresh snapshot from current domain state, validates it through the same parser boundary, and downloads formatted JSON. Data export calls `deriveTrackKinematics` for every track, preserving the application’s raw timestamp and derivative policy regardless of the session-only scientific view. CSV rows are globally chronological and carry stable IDs, frame references, native pixels, generic analysis values, and explicit unit fields. JSON data export omits workspace-only state. Graph export runs the current visualization group through the tested chart layout and emits a self-contained SVG with title, legend, axes, and units. Motion views include measured/smoothed/model layers as applicable; residual views include selected-source observation markers, potential-outlier rings, and an explicit zero baseline for signed components. Neither includes playhead, cursor, or hit targets.

For reports, `VideoWorkspace` prepares one raw or selected smoothed analysis per track using the existing analysis functions, then passes any existing model fit and `FitDiagnostics` into `buildExperimentReport`. This per-track work is deferred until the Report view opens, and the report presentation module is lazy-loaded. The builder only aggregates/selects: endpoint displacement divided by duration defines average velocity, existing cumulative distance divided by duration defines average speed, and maximum values select existing finite derived magnitudes. `graphs.ts` filters existing `VisualizationGroup`s rather than implementing a chart. Both the React document and `reportHtml.ts` consume the same typed model; HTML contains embedded CSS/SVG, has no script/network dependency, and excludes the video.

Meaningful annotations, calibration, or tracks activate lightweight discard confirmation for video replacement/removal and opening another valid project. Failed parsing and failed export generation report that existing work remains safe. Playwright generates a synthetic local WebM inside Chromium and covers save, open/relink/restore, CSV and SVG download, malformed-project safety, mismatch warning, and canceled destructive removal without external services.
...
The Report view replaces the active analysis grid without unmounting experiment state. Its dark configuration rail and light document preview scroll independently on desktop and stack without horizontal overflow at the existing responsive breakpoint. Print media hides the application header, workspace navigation, configuration, and editing controls; the light report becomes normal-flow content with printable SVGs, wrapping tables, and controlled page breaks.

The application header retains a compact Help button at every product state. Native modal dialogs constrain height and width, restore trigger focus, close with Escape, and stack Help navigation above content on narrow screens. The active analysis grid declares zero-minimum tracks at responsive widths so child control minimums cannot cause document-level horizontal overflow.

## Experiment report

Report metadata and inclusion preferences are scientifically inert project state and do not enter annotation/tracking history. Report model construction is deterministic for an explicit generation timestamp and uses the active calibration plus the current Raw/Smoothed/model controls across every track. A model section exists only where the existing fit and matching Phase 11 diagnostics are both available. Potential deviations copy only Phase 11 visual flags and retain the same caution: they are not measurement-error classifications and never alter fitting or observations.

The observation table retains full numeric precision in the model while presentation uses the existing human-readable formatter and an em dash for `null`. The standalone report has report schema version 1 independently inside the additive MotionLab project section. Report HTML is presentation output, not a reopenable scientific project and not a replacement for CSV/JSON.
...
`useAssistedTracking` owns transient `idle`, `seed-selecting`, `seeded`, `running`, `stopped`, `failed`, and `completed` session states. Each run records the seed, native resolution, selected geometry, current position, transient proposals, processed count, latest confidence, consecutive misses, whether the latest search used motion guidance, elapsed time, average milliseconds/frame, and a session identity. A bounded 120-entry internal diagnostic trail records frame/time, previous and predicted positions, recent displacement and velocity, primary and fallback bounds/confidences, selected pass, best position, cluster count, recovery attempt, template source, acceptance, and failure reason without adding production UI. None of this diagnostic geometry or guidance is written to `TrackSample`. `AbortController` plus a generation token prevents late seek/worker results from mutating a stopped, replaced, or newer session. Playback, active-track change, incompatible canvas interaction, video replacement, and unmount cancel in-scope work safely.

The first assisted frame uses the existing seed-centered coarse envelope without prediction. After two image-confirmed observations exist, `motionGuidance.ts` derives constant velocity from their native-coordinate displacement and real media-time interval, then projects that velocity to the next timestamp when the interval ratio is between 0.25 and 4. Predictions are capped at 512 native pixels and clamped safely to the frame; invalid timing or insufficient history preserves the original unguided behavior.

`adaptiveSearch.ts` converts valid projected motion into two bounded geometries. The primary native radius is `base + 0.35 × projected motion + 0.25 × template size`, aligned to the coarse scale and capped at the smaller of 1.5× base or 512 px. It searches only around the predicted center. Only when that pass returns low confidence does the controller run one same-frame fallback over the previous-to-predicted corridor, using `base + projected motion + template size`, aligned and capped at 512 px. Match scores, confidence thresholds, basin ambiguity rules, and sample acceptance are unchanged. A successful primary pass skips fallback; prediction alone never creates a sample.

If both same-frame passes remain uncertain, the Phase 7.4 continuity policy advances a separate processed-frame cursor while leaving reliable observations and the physical anchor unchanged. Subsequent recovery attempts project those reliable observations to the later media time and expand the adaptive predicted-center geometry to approximately 1.35×, 1.7×, then 2×, still capped at 512 px. Three misses are tolerated; a fourth consecutive miss terminates recovery. Reacquisition creates a real image-confirmed proposal at its actual timestamp, so missed timestamps remain genuine gaps.
...
`VideoWorkspace` memoizes raw full-track analysis on active track/calibration identity, then memoizes the optional smoothed source and selected global model from that scientific state. Playback timestamp updates only select the current derived sample and do not recompute regressions. A track edit, delete, undo/redo, active-track switch, calibration change, source/window change, or model change updates the affected derived quantities and graph layers immediately.

Visualization selectors expose three dimensionally coherent motion groups: Position contains X and Y, Velocity contains vx, vy, and Speed, and Acceleration contains ax, ay, and magnitude. `series.ts` creates these immutable groups from `TrackKinematics`, preserving source sample IDs/timestamps and omitting each unavailable derivative independently. In smoothed mode it adds secondary raw-observation series, and for an active model it evaluates already-fitted coefficients at a fixed display-only timeline. Those model points have no tracking identity or hit targets. Residual selectors instead expose one observation-aligned Magnitude, X, or Y marker series from `FitDiagnostics`; they never interpolate between residuals. View, quantity/component, source/window, model, and dock state live in a tiny reducer above the panel and remain outside project/tracking history.
...
The responsive SVG labels media time and the reactive dimensional unit explicitly and draws a distinct zero baseline whenever the displayed range crosses zero. Circle, square, and diamond markers plus labels distinguish components without relying on color. Raw-only mode retains its unconnected observation markers; smoothed values may be joined as a derived trajectory while hollow raw markers stay visible, and analytic models use distinct dashed paths. The actual `controller.currentTime` drives an unsnapped video playhead, while pointer movement owns a transient time-only cursor; arbitrary cursor positions never produce interpolated quantities. Background clicks pause and seek by mapped timestamp. Observation-marker activation stops propagation and seeks to the sample's exact anchor, and the existing frame-reference selector—not floating-point equality—continues to determine active-marker highlighting.

Phase 6 intentionally retained the hand-authored SVG rather than adding a chart dependency. Phase 9 extends the same tested layout with explicit measured, smoothed, and model layers without adding a chart library or persistence boundary.

Numerical differentiation amplifies tracking noise. Velocity is less stable than position, and acceleration—being a second numerical derivative—can be substantially noisier. Raw preserves that original policy. Optional Phase 9 smoothing can reduce noise but may hide real rapid changes, so the raw comparison remains visible.

## Scientific smoothing and motion-model fitting

`leastSquares.ts` constructs the small normal systems needed for degree-1/2 polynomial fits and solves them with deterministic partial pivoting. Time is always relative to the local/model reference and normalized before solving; sub-microsecond spans, singular pivots, invalid inputs, and non-finite outputs return `null`. No React, DOM, canvas, worker, storage, network, chart, or numerical dependency enters this math boundary.

`deriveSmoothedTrackKinematics` first reuses raw analysis to obtain validated chronological observations in the active pixel/world coordinate space. For each original timestamp it selects the nearest available 5/7/9 observations by absolute time proximity with deterministic ties, fits X and Y quadratics, and evaluates `c0`, `c1`, and `2c2`. The shared kinematics constructor then derives displacement/path semantics from those smoothed positions. The typed output retains the original `TrackSample`, raw native position, and raw analyzed position beside explicitly named smoothed values. Fewer than five samples or any unusable local window makes the selected derived source unavailable; it never mutates or silently substitutes raw data.

`fitMotionModel` performs one global degree-1 or degree-2 fit per axis against the selected raw/smoothed positions, using exact timestamps relative to the earliest reported `t0`. It returns coefficients in physical coordinates, spatial RMSE, per-axis R² or `null` for zero-variance axes, sample count, and time span. `evaluateMotionModel` cheaply evaluates those coefficients for graph presentation. Fits recompute only when their memoized scientific inputs or controls change and never enter track history, undo/redo, Assisted Tracking, CSV/JSON, or project version 1.

## Fit diagnostics and residual analysis

`deriveFitDiagnostics` accepts the selected analysis source and its matching `MotionModelFit`, evaluates the fit once at every genuine source observation, and creates an immutable record containing observed position, predicted position, signed X/Y residuals, magnitude, sample identity, and exact media timestamp. It rejects source/count mismatches and non-finite inputs instead of presenting stale or mixed-space diagnostics. `VideoWorkspace` memoizes this projection beside the model fit, so track edits, deletes, undo/redo, calibration changes, active-track changes, source changes, and model changes rebuild diagnostics automatically; playback and diagnostic selection do not.

The diagnostic summary directly reuses the fit's spatial RMSE and R². Spatial MAE is the mean Euclidean residual magnitude, maximum residual is its maximum, and mean X/Y retain direction. Rankings are deterministic by descending magnitude, then time and sample identity. The visual potential-outlier rule is enabled only for at least seven finite magnitudes and uses `median + 4 × 1.4826 × MAD` with a strict greater-than comparison. A zero/non-finite MAD yields no flags. Flags never exclude observations from the fit, change weights, edit tracks, or enter persistence/export data.

The inspector's largest-deviation buttons reuse the normal exact-sample seek path. Selected details expose observed/model coordinates and the residual vector; correction remains the existing Tracking Edit or Delete workflow, preserving one source of truth and ordinary tracking history. The Analysis dock reuses `KinematicsGraph` for signed X, signed Y, or magnitude residual markers. Signed graphs and standalone SVGs include a zero reference; outlier rings are diagnostic decoration around the real seekable observation marker. Residual series have no fitted connecting line, fake samples, or model curve.
