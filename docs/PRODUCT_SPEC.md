...
An active model also exposes fit diagnostics derived at those same genuine observation timestamps. For each point, the residual is observed minus predicted position in the selected raw/smoothed and pixel/world analysis space. The inspector reports spatial RMSE, spatial MAE, maximum residual magnitude, mean signed X/Y residuals, and deterministic largest-deviation rankings. A Residuals graph provides magnitude and signed-component views with exact-anchor seeking; users review the corresponding video point and correct it through ordinary Tracking Edit or Delete. Conservative MAD flags are visual review cues only and never remove or reweight observations. A poor model can create large residuals even when the measurements are valid.

The Report workspace turns the current experiment state into a print-oriented document without scraping UI or changing scientific data. It supports optional title/author/date/course/instructor/description/discussion metadata, per-track inclusion, selected reusable motion/fit/residual graphs, optional observation tables, measurement summaries, current model diagnostics, and factual provenance. The browser print workflow produces PDF-ready output, and standalone HTML embeds all styles and generated SVG graphs for offline use without embedding the local video. Interpretation remains explicitly user-written.
...
The version-1 project schema contains the original video name with optional duration and native dimensions, annotations and frame references, calibration and coordinate-axis metadata, confirmed tracks/samples, active-track identity, trail and mark-advance preferences, analysis mode/collapse state, media time, and an additive validated report section. The report section stores only user-authored metadata plus excluded-track, selected-graph, and observation-table preferences. It excludes derived summaries, graph data, fit results, residuals, Object URLs, video bytes, undo stacks, pointer drafts, worker execution, templates, predictions, diagnostics, unaccepted Assisted Tracking suggestions, Getting Started status, and inspector disclosure state. Pre-Phase-12 files without `report` normalize to default settings. Parsing remains read-then-validate; only a valid project can replace the live workspace.
...
CSV is one chronological multi-track table. Native pixel coordinates are always present. Generic analysis columns are paired with explicit `position_space`, `position_unit`, `velocity_unit`, and `acceleration_unit` fields, so calibrated world values and uncalibrated pixel values are unambiguous. Missing derivatives are empty. Scientific JSON contains the same derived sample quantities plus calibration/annotation context but omits workspace-only metadata. CSV and scientific JSON remain based on confirmed observations and the unchanged raw kinematics path even when a derived scientific view is selected. Graph SVG reflects the current selected motion or residual visualization and excludes interactive chrome. Source, smoothing window, model, graph view, residual component, diagnostics, rankings, and outlier flags are session-derived and do not alter the version-1 project schema.

Experiment-report HTML is a separate presentation export. It consumes the selected report model, embeds styles and portable SVG graphs, includes no script or external runtime dependency, and never embeds video. Measurement summaries use the selected Raw/Smoothed analysis source consistently. Initial/final positions and cumulative path distance come from existing kinematics; average velocity is endpoint displacement divided by tracked duration, average speed is cumulative path distance divided by duration, and maxima select existing finite speed/acceleration samples. Missing science stays unavailable rather than falling back silently.
...
Fit diagnostics evaluate that selected model at each selected-source observation. With residual components `dx = observed.x - predicted.x` and `dy = observed.y - predicted.y`, residual magnitude is `sqrt(dx² + dy²)`, spatial RMSE remains `sqrt(mean(dx² + dy²))`, spatial MAE is `mean(sqrt(dx² + dy²))`, and maximum residual is the largest magnitude. Largest deviations sort by descending magnitude with timestamp and stable sample identity as deterministic ties. Potential outliers require at least seven finite residual magnitudes and use a strict upper threshold of `median + 4 × 1.4826 × MAD`; zero/invalid MAD or smaller samples produce no flags. This rule is intentionally conservative, one-sided, and visual-only. Diagnostics recompute after source, model, tracking, or calibration changes and never mutate samples or select a scientific model on the user's behalf.
